...
- Injection: SQL, NoSQL, XSS, command, header, template (SSTI), LDAP, and prompt injection via any content an LLM reads
- SSRF: any server-side fetch of a user-supplied URL (webhooks, image/PDF fetchers, link previews, import-from-URL, OAuth redirect discovery) reaching internal metadata endpoints or private hosts
- Path traversal & arbitrary file access: user-controlled paths in upload/download/read (`../`, absolute paths, null bytes, symlinks)
- Unsafe deserialization: `pickle`, `yaml.load`, PHP `unserialize`, Java/Ruby object streams, and JS prototype pollution from merged user objects
- XXE: XML parsers with external entities enabled on any user-supplied XML/SVG/docx
- Open redirects & unvalidated forwards (`?next=`, `returnUrl=`, OAuth `redirect_uri`)
- Crypto misuse: MD5/SHA1 or fast hashes for passwords, JWT `alg:none` or unverified signature, hardcoded/predictable secrets, ECB mode, static IVs, non-constant-time token/HMAC comparison, `Math.random()` for tokens
- Mass assignment / over-posting: `req.body` spread into an ORM create/update letting a user set `role`, `isAdmin`, `ownerId`, `price`
- Secrets: keys in code, tokens in logs, credentials in client bundles, secrets in git history
- Unverified webhooks / missing signature or timestamp (replay) checks
- Business-logic & money: prices/quantities/discounts trusted from the client, negative amounts, integer overflow, float currency and rounding drift, coupon/referral abuse, workflow steps skippable out of order
...
> `[CRITICAL] [SECURITY] src/api/preview.ts:14 - fetches any user-supplied url server-side; http://169.254.169.254/ returns cloud credentials (SSRF) - allowlist hosts, block link-local and private ranges`
...
## 25. Live testing (runtime)
Only when a local instance is reachable. Drive the running product like a QA engineer: hit every endpoint, submit every form with valid/empty/oversized/malformed input, run the auth matrix across real sessions (replay each request as every role, change ids for IDOR), fire bounded probes for missing rate limits and concurrency races, upload hostile files, and inspect security headers and error surfaces. Full method and hard safety rails (localhost-only, dev-DB-only, disposable marked test data, never production) in [live-testing.md](live-testing.md). Findings here are `runtime-confirmed` and carry the exact reproducing request as evidence; findings the running server disproves are `runtime-cleared` and dropped.

> `[HIGH] [SECURITY] POST /api/auth/reset - 40 requests in 3s all returned 200 with no throttling; password-reset email is an unmetered spam cannon (runtime-confirmed) - rate-limit per account and IP`
